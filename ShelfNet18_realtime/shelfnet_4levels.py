...
class ShelfNet(nn.Module):
    def __init__(self, n_classes, *args, **kwargs):
        ...
        self.trans2 = ConvBNReLU(256*4, 256, ks=1, stride=1, padding=0)
        self.trans3 = ConvBNReLU(512*4, 512, ks=1, stride=1, padding=0)
    def forward(self, x, aux = True):
        ...
        feat4 = self.trans0(feat4)
        feat8 = self.trans1(feat8)
        feat16 = self.trans2(feat16)
        feat32 = self.trans3(feat32)

        out = self.decoder([feat4, feat8, feat16, feat32])

        out2 = self.ladder(out)

        feat_cp4, feat_cp8, feat_cp16, feat_cp32 = out2[-1], out2[-2], out2[-3], out2[-4]
